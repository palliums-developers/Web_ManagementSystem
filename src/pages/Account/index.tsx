    ...
    <PageContainer className={styles.main}>
      {userInformation && (
        <div>
          ...
          >
            <p>
              {intl('account.originalPassword')}
              <Input.Password
                placeholder={intl('account.enterPassword')}
                onChange={(e) => handleChange('old_password', e.target.value)}
              />
            </p>
            <p>
              {intl('account.newPassword')}
              <Input.Password
                placeholder={intl('account.passwordLimit')}
                onChange={(e) => handleChange('new_password', e.target.value)}
              />
            </p>
            <p>
              {intl('account.confirmPassword')}
              <Input.Password
                placeholder={intl('account.passwordConfirm')}
          ...
          <Modal
            ...
            onCancel={() => handleModalCancel()}
            title={
              userInformation.phone ? intl('account.modifyPhone') : intl('account.settingPhone')
            }
          >
            <p>
              {intl('account.phone')}
              ...
              <Input
                ...
                addonAfter={
                  <span onClick={getCAPTCHA} style={{ cursor: 'pointer' }}>
                    {captchaTime === 0 ? intl('account.getCAPTCHA') : captchaTime + 'S'}
                  </span>
          ...
          <Card>
            ...
            <div className={styles.information}>
              <p>{intl('account.phone')}</p>
              <span onClick={() => showModal('phone')}>
                {userInformation.phone ? intl('account.modify') : intl('account.set')}
              </span>
            </div>
            <div className={styles.information}>
