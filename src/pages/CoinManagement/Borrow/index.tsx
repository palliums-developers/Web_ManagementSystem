...
export default () => {
  ...
  const handleModalOk = async (type: string) => {
    ...
    switch (type) {
      case 'status':
        ...
        result = await postBankProduct(type, 'borrow', postData)
        break;
      case 'add':
        break;
      case 'edit':
  ...
  const initial = async () => {
    const temp = await getBankProduct('data','borrow');
    setRawData(temp);
    setShowData(getShowDataFromRawData(temp));
  }
  ...
  const columns: any = [
    ...
    {
      title: intl('bank.min_borrow'),
      dataIndex: 'min',
      key: 'min_borrow'
    },
    {
      title: intl('bank.daily_borrow'),
      dataIndex: 'max',
      key: 'daily_borrow'
    },
    {
      title: intl('bank.step_borrow'),
      dataIndex: 'increase',
      key: 'step_borrow'
    },
    {
